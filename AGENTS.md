...
- `src/backends/` has one implementation per model id prefix: `claude/*` spawns `claude -p` (`claude.ts`), `openrouter/*` calls the OpenRouter HTTP API (`openrouter.ts`). `registry.ts` parses ids. The `Backend` interface is in `backends/types.ts`. The Claude backend never sets `CLAUDE_CODE_MAX_OUTPUT_TOKENS` (an overrun there is a hard error) and uses `stream-json` output for tool runs so a max-turns cap keeps the last assistant text.
